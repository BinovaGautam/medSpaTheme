...
class SemanticColorSystem {
    ...
    generateCSSProperties(paletteId) {
        ...
        // CRITICAL FIX: Check if server CSS variables already exist
        // If they do, don't override them with JavaScript
        const rootStyle = getComputedStyle(document.documentElement);
        const serverHasCSSVars = rootStyle.getPropertyValue('--component-bg-color-primary').trim() !== '';

        if (serverHasCSSVars) {
            console.log('🖥️ Server CSS variables detected - skipping JavaScript override');
            console.log('🖥️ Server primary color:', rootStyle.getPropertyValue('--component-bg-color-primary').trim());
            console.log('🖥️ Respecting server-side CSS generation over client-side');
            return '';
        }

        console.log('⚠️ No server CSS detected - generating client-side CSS as fallback');

        // Original CSS generation logic continues...
        const colors = palette.colors;
        const properties = [];

        // Generate CSS properties only as fallback
        Object.entries(colors).forEach(([role, colorData]) => {
            const hex = colorData.hex || colorData;
            const cssVar = `--color-${role}`;
            properties.push(`${cssVar}: ${hex};`);

            // Add component tokens as fallback
            if (role === 'primary') {
                properties.push(`--component-bg-color-primary: ${hex};`);
                properties.push(`--component-border-color-primary: ${hex};`);
            }
            if (role === 'secondary') {
                properties.push(`--component-bg-color-secondary: ${hex};`);
            }
            if (role === 'accent') {
                properties.push(`--component-bg-color-accent: ${hex};`);
            }
        });
